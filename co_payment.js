...
window.addEventListener("load", function() {
   ...
   // split the formData string anywhere that there is an "=" or an "&"
...
window.addEventListener("load", function() {
   ...
   document.getElementsById("cardNumber").oninput = validateNumber;
   ...
   function runSubmit() {
      //this function runs all seoerate validation based function
      validateName();
      validateCredit();
      validateNumber();
      validateMonth();
...
function validateName() {
   ...
   if (cardName.validity.valueMissing) {
      cardName.setCustomValdity("Enter your name as it appears on the card");
   } else {
      cardName.setCustomValdity("");
   }

}
// definiton of the validateCredit() function
function validateCredit() {
   let creditCard = document.forms.payment.elements.credit[0];
}

//defintion of validateNumber() function
function validateNumber() {
   let cardNumber = document.getElementById("cardNumber");
   if (cardNumber.validity.valueMissing) {
      cardNumber.setCustomValdity("Pleasse eneter your card number!");
   } else if (cardNumber.validity.patternMismatch) {
      cardNumber.setCustomValdity("Please enter a valid card number!");
   } else {
      cardNumber.setCustomValdity("");
   }
}
...
function validateMonth() {
   ...
   if (cardMonth.selectedIndex === 0) {
      cardMonth.setCustomValdity("Please select the expiration month!")
   } else {
      cardMonth.setCustomValdity("");
   }
}
...
function validateYear() {
   ...
   if (cardYear.selectedIndex === 0) {
      cardYear.setCustomValdity("Please select the expiration month!")
   } else {
      cardYear.setCustomValdity("");
   }
}

// defintion of the validateCVC() function
function validateCVC() {
   ...
   } else if((creditCard.value === "amex") && (/^\d{4}$/.test(caardCVC.value) === false)) {
      cardCVC.setCustomerValidity("Please enter a 4-digit CVC number!");
   } else if((creditCard.value !== "amex") && (/^\d{3}$/.test(cardCVC.value) == false)) {
      cardCVC.setCustomValdity("Please eneter a 3-digit CVC number!");
   } else {
      cardCVC.setCustomValdity("");
   }
}